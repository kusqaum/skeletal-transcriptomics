...
##### read in data sets####
networkAlone <- readRDS("processed/processedNetworkEmb.rds") # this is to do ML on network alone
networkAloneLabelled <- readRDS("processed/processedNetworkEmbLabelled.rds") # labelled with Skeletal phenotype
sVwithNetworkLabelled <- readRDS("processed/SVwithNetworkLabelled.rds")
concatNMFsvNetwk <- readRDS("processed/concatNMFsvNetwk.rds") # this is doing ACTUALLY concatenating like it was supposed to be
labelsAlone <- sVwithNetworkLabelled[[1]] %>% select(significant)
mortalityLabel <- read.table("processed/processedMortalityLabels.txt", header = T, sep = "\t")
mortalityLabel$significant <- as.factor(mortalityLabel$significant) 

head(sVwithNetworkLabelled[[1]][1:5,1:5])
dim(sVwithNetworkLabelled[[1]])

#now adding step to make network data integrated with mortality labels to see how
# well the network alone can predict this phenotype
netMortal <- merge(networkAlone, mortalityLabel, by = 0); rownames(netMortal) <- netMortal$Row.names; netMortal$Row.names <-NULL
...
networkAloneSplitMortality <- split_processingData_fctn(netMortal, 0.8)
concatNMFsvNetwkSplit <- lapply(concatNMFsvNetwk,split_processingData_fctn, 0.8)
...
randForest_fctnConcatNetwork <- function(preprocessResult, algorithm){
  print(paste0("Number of predictors = ", ncol(preprocessResult$train)-1))
  if(algorithm == "NMF"){
    ...
    result <- list("workflow" = wkflow,"res"= res, "resDf" = resDf, "finalMod" = final_model, "tuningPlots" =paramsPlot, "importancePlot"=importancePlot, "importanceDf"=importanceDf,
                   "finalFit" = final_fit, "AUC"= roc_auc, "roc_curve" = rocCurve, "aug"= aug_m, "algorithm" = algorithm,
                   "dim" = dimension, "dfForCorrectR" = df)
    saveRDS(result, sprintf("processed/%sRFResNetworkConcatWithSV_%s.rds",algorithm, (ncol(preprocessResult$train)-1)))
...
xgboost_fctnNetworkAlone <- function(preprocessResult){
  ...
  tScore <- qt(p = alpha, df = dof, lower.tail = F)
  resDf <- res %>% collect_metrics() %>%
    mutate(dim = ncol(preprocessResult$train)-1, Algorithm = "none", marginError = std_err* tScore) %>%
    mutate(lowerbound = mean - marginError, upperbound = mean + marginError)
  ...
  saveRDS(result, sprintf("processed/xgbResMortalityNetworkOnly_%s.rds", ncol(preprocessResult$train)-1))
...
xgboost_fctnConcatNetwork <- function(preprocessResult, algorithm) {
  paste0("Number of predictors = ",print(ncol(preprocessResult$train)-1))
  if(algorithm == "NMF"){
    ...
    result <- list("workflow" = wkflow, "res" = res, "resDf" = resDf, "finalMod" = final_model,
                   ...
                   "finalFit" = final_fit, "AUC"= roc_auc, "roc_curve" = rocCurve,"confMat" = confMat,
                   "aug"= aug_m, "algorithm"=algorithm, "dim" = dimension, "dfForCorrectR" = df)
    saveRDS(result, sprintf("processed/%sxgbResConcatSV_%s.rds",algorithm, ncol(preprocessResult$train)-1))
...
svWithNetwkRFres <- lapply(sVwithNetworkSplit, algorithm = "NMF")
networkAlonePredictSPxgb <- xgboost_fctnNetworkAlone(networkAloneSplit)

# concat200dimNMF <- readRDS("processed/concat200dim.rds")
# concatSplit200 <- split_processingData_fctn(concat200dimNMF, proportion = 0.8) 

# cl <- makeCluster(25, outfile="processed/1000dimNMF_NetworkConcatXGB.txt")
# registerDoParallel(cl)
# print("xgb on 1000 dim NMF SkeletalVis concatenated with network:")
# # nmfNetworkdim700Res <- randForest_fctnConcatNetwork(concatNMFsvNetwkSplit[[6]], algorithm = "NMF")
# nmfNetworkdim1000Res <- xgboost_fctnConcatNetwork(concatNMFsvNetwkSplit[[7]], algorithm = "NMF")

# 

# c <- makeCluster(25, outfile="processed/xgbMortalityNetworkOnly.txt")
# registerDoParallel(c)
# print("xgb on network data alone")
# netRes <- xgboost_fctnNetworkAlone(networkAloneSplitMortality)
