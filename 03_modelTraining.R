...
randForest_fctn <- function(preprocessResult, algorithm){
  ...
  if(algorithm == "NMF"){
    ...
    saveRDS(result, sprintf("processed/%sRFResSV_%s.rds",algorithm, (ncol(preprocessResult$train)-1)))
...
xgboost_fctn <- function(preprocessResult, algorithm) {
  ...
  if(algorithm == "NMF"){
    ...
    saveRDS(result, sprintf("processed/%sxgbResSV_%s.rds",algorithm, ncol(preprocessResult$train)-1))
...
nmfDataMlResList_RF <- lapply(nmfProcessed, FUN=randForest_fctn, algorithm="NMF")
nmfDataMlResList_XGB <- map(.x = nmfProcessed, .f = xgboost_fctn, algorithm = "NMF")
pcaDataMlResList_XGB <- map(.x = pcaProcessed, .f = xgboost_fctn, algorithm = "PCA")

# randomise the 200 dimension/500 dimension and train xgb
single200dimDfNMF<- nmfDfs[[6]]
set.seed(511) # set seed for shuffling since this is a random process
shuffled200dimNMF <- transform(single200dimDfNMF, significant = sample(single200dimDfNMF$significant))
shuffled200dimNMFSplit <- split_processingData_fctn(shuffled200dimNMF, 0.8)



print(xgboost_fctn)
print("set the number of cpus to use in parallel")
clx <- makeCluster(25, outfile="processed/50dimNMF_XGB_noNetwork.txt")
registerDoParallel(clx)
print("xgboost on 50 dim nmf SkeletalVis only:")

resultshuffled <- xgboost_fctn(shuffled200dimNMFSplit, algorithm = "NMF")
saveRDS(resultshuffled, "processed/NMFxgbResSVshuffled_200.rds")


# cl <- makeCluster(25, outfile="processed/500dimNMF_RF_noNetwork.txt")
# registerDoParallel(cl)
# print("randforest on 500 dim NMF SkeletalVis only:")
# nmfdim500Res <- randForest_fctn(allsplitsNMF[[7]], algorithm = "NMF")
