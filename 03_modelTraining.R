...
pcaDfs2 <- readRDS("processed/pcaDataframes_tuned.rds")
...
temporaryNMFlist <- list()
nmfSplit1 <- split_processingData_fctn(nmfDfs[[1]], 0.8)
nmfRes1 <- justToTestRF(nmfSplit1, algorithm = "NMF")
nmfSplit2 <- split_processingData_fctn(nmfDfs[[2]], 0.8)
nmfRes2 <- justToTestRF(nmfSplit2, algorithm = "NMF")
nmfSplit3 <- split_processingData_fctn(nmfDfs[[3]], .8)
nmfRes3 <- justToTestRF(nmfSplit3, algorithm = "NMF")
nmfSplit4 <- split_processingData_fctn(nmfDfs[[4]],.8)
nmfRes4 <- justToTestRF(nmfSplit4, algorithm = "NMF")
nmfSplit5 <- split_processingData_fctn(nmfDfs[[5]], .8)
nmfRes5 <- justToTestRF(nmfSplit5, algorithm = "NMF")
nmfSplit6 <- split_processingData_fctn(nmfDfs[[6]], 0.8)
nmfRes6 <- justToTestRF(nmfSplit6, algorithm = "NMF")
...
xgboost_fctn <- function(preprocessResult, algorithm) {
  ...
  if(algorithm == "NMF"){
    ...
    model_xgb <- boost_tree(trees = tune(), tree_depth = tune(), mtry=tune(), min_n = tune(), learn_rate = tune(),
                            sample_size = tune(),
    ...
    model_xgb <- boost_tree(trees = 1000, tree_depth = tune(), mtry=tune(), min_n = tune(),
                            learn_rate = tune(),
                            sample_size = tune(),
                            loss_reduction = tune(), mode = "classification") %>%
      set_engine("xgboost")
...
testingXgboost_fctn <- function(preprocessResult, algorithm) {
  if(algorithm == "NMF"){
    model_xgb <- boost_tree(trees = tune(), tree_depth = tune(), mtry=tune(), min_n = tune(), learn_rate = tune(),
                            sample_size = tune(),
                            loss_reduction = tune(), mode = "classification") %>%
...
nmfXGBRes1 <- testingXgboost_fctn(nmfSplit1, algorithm = "NMF")
nmfXGBRes2 <- testingXgboost_fctn(nmfSplit2, algorithm = "NMF")
nmfXGBRes3 <- testingXgboost_fctn(nmfSplit3, algorithm = "NMF")
print("PCA datasets")
dataCR <-nmfXGBRes1$dfForCorrectR
dataCR$model <- "XGB"
xgbrf<- rbind(nmfRes1$dfForCorrectR, dataCR)
corrRes<- repkfold_ttest(data = xgbrf, n1=80, n2=20, k=3, r=2)
corrRes




##okay so gonna do each dimension one by one:
pcaDfs2_1 <- pcaDfs2[[7]]
print("split data and create recipe")
split_1 <- split_processingData_fctn(pcaDfs2_1, 0.8)
print("random forest on 28 dimension SkeletalVis data")
rf_res1 <- randForest_fctn(split_1, algorithm = "PCA")
print("finished")
#-----------------------------------------------------------------------------------------------------------------------
